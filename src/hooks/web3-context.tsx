...
import {Networks} from "helpers/bond/constants";
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
    ...
    const [chainID] = useState(Networks.HARMONY);
    const [providerChainID, setProviderChainID] = useState(Networks.HARMONY);
    const [address, setAddress] = useState("");

    const [uri] = useState("https://api.s0.t.hmny.io");
    ...
    const checkWrongNetwork = async (): Promise<boolean> => {
        if (providerChainID !== Networks.HARMONY) {
