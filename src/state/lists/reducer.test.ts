      ...
      beforeEach(() => {
        store = createStore(reducer, {
          byUrl: {
            ...
            'https://unpkg.com/@foxswap/default-token-list@latest': {
              error: null,
              current: STUB_TOKEN_LIST,
              loadingRequestId: null,
              pendingUpdate: null
    ...
    describe('initialized with a different set of lists', () => {
      beforeEach(() => {
        store = createStore(reducer, {
          ...
          activeListUrls: undefined,
          lastInitializedDefaultListOfLists: ['https://unpkg.com/@foxswap/default-token-list@latest']
        })
        store.dispatch(updateVersion())
      })
      ...
      it('each of those initialized lists is empty', () => {
        ...
        Object.keys(byUrl).forEach(url => {
          if (url !== 'https://unpkg.com/@foxswap/default-token-list@latest/uniswap-default.tokenlist.json') {
